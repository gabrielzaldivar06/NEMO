#!/usr/bin/env python3
"""
NEMO Installer v1.1.0  —  macOS
macOS bootstrapper: installs NEMO and auto-configures all detected MCP clients.

Build:
    pip3 install pyinstaller
    pyinstaller --onefile --windowed --name "NEMO Installer" nemo_installer_mac.py
    # → dist/NEMO Installer.app  (double-click to run)
"""
...
from tkinter import (
    BooleanVar, Button, Canvas, Checkbutton, DISABLED, DoubleVar, END,
    Entry, Frame, Label, LEFT, NORMAL, RIGHT, Scrollbar, StringVar,
    Text, Tk, Toplevel, VERTICAL, WORD, X, Y, FLAT, BOTH, filedialog,
    messagebox, ttk,
)
...
# ── Embedded icon (same 64×64 PNG-in-ICO as Windows version) ──────────────
# The PNG is extracted at runtime from inside the ICO at offset 22.
...
# ── Editor definitions (macOS paths) ──────────────────────────────────────
...
EDITORS = {
    "VS Code": {
        "icon"   : "💻",
        "detect" : HOME / "Library/Application Support/Code/User",
    ...
    "Windsurf": {
        "icon"   : "🌊",
        "detect" : HOME / "Library/Application Support/Windsurf/User",
        "config" : HOME / "Library/Application Support/Windsurf/User/mcp.json",
        "format" : "vscode",
        "restart": None,
    },
    "Claude Desktop": {
        "icon"   : "🤖",
        "detect" : HOME / "Library/Application Support/Claude",
...
def _find_brew() -> str | None:
    """Return path to brew binary, or None if Homebrew is not installed."""
...
def brew_install_python() -> bool:
    """Attempt Python 3.12 install via Homebrew. Returns True on success."""
    brew = _find_brew()
    if not brew:
        return False
    try:
...
def find_python() -> str | None:
    """Return path to a Python 3.10+ interpreter, or None."""
    candidates: list[str] = []

    # 1. Homebrew standard locations (Apple Silicon + Intel)
    for brew_dir in ("/opt/homebrew/bin", "/usr/local/bin"):
        for name in ("python3.12", "python3.11", "python3.10", "python3"):
            p = Path(brew_dir) / name
            if p.exists():
    ...
    # 3. Python.org framework installs
    for ver in ("3.12", "3.11", "3.10"):
        p = Path(f"/Library/Frameworks/Python.framework/Versions/{ver}/bin/python3")
...
def write_mcp_config(
    ...
    if fmt == "vscode":
        ...
        existing.setdefault("mcpServers", [])
        existing["mcpServers"] = [
            s for s in existing["mcpServers"] if s.get("name") != "nemo"
        ]
        ...
        existing["context_servers"]["nemo"] = {
            "command": {"path": python_exe, "args": [server_script]},
        }
...
class StepItem(Frame):
    ...
    _SPIN   = ["◐", "◓", "◑", "◒"]
    _COLORS = {
        "pending": MUTED, "running": BLUE,
        "done"   : GREEN, "error"  : RED, "skip": MUTED,
    }
    ...
    def __init__(self, parent: Frame, label: str) -> None:
        ...
        self._icon_lbl = Label(
            ...
            bg=SURFACE, fg=MUTED, font=(FONT_UI, 12), width=2,
        )
        self._icon_lbl.pack(side=LEFT, padx=(4, 8))
        self._text_lbl = Label(
            self, text=label,
            bg=SURFACE, fg=MUTED, font=(FONT_UI, 10), anchor="w",
        )
...
class NemoInstaller(Tk):
    ...
    def __init__(self) -> None:
        ...
        # Set app icon — extract PNG from embedded ICO (offset stored at bytes 18-21)
        try:
            from tkinter import PhotoImage
            ico_data   = base64.b64decode(ICON_B64)
            img_offset = struct.unpack_from("<I", ico_data, 18)[0]
            png_data   = ico_data[img_offset:]
            png_path   = Path(tempfile.gettempdir()) / "nemo_installer.png"
            png_path.write_bytes(png_data)
            self._icon_img = PhotoImage(file=str(png_path))  # keep ref to prevent GC
    ...
    def _build_header(self) -> None:
        ...
        Label(brand, text="NEMO", bg=SURFACE, fg=TEXT,
              font=(FONT_UI, 19, "bold")).pack(anchor="w")
        ...
        Label(trust, text="🔒  100% Local · Sin nube · Sin suscripciones",
              bg=SURFACE2, fg=GREEN, font=(FONT_UI, 9, "bold")).pack()
        ...
        def _draw_line():
            ...
            for i in range(steps):
                ...
                line.create_rectangle(x0, 0, x1, 3,
    ...
    def _feature_card(
        ...
        Label(top_row, text=icon, bg=SURFACE,
              font=(FONT_UI, 15), width=2).pack(side=LEFT)
        Label(top_row, text=headline, bg=SURFACE, fg=TEXT,
              font=(FONT_UI, 10, "bold"), anchor="w").pack(side=LEFT, padx=(8, 0))
        Label(inner, text=detail, bg=SURFACE, fg=MUTED,
              font=(FONT_UI, 9), anchor="w").pack(anchor="w", padx=(26, 0), pady=(2, 0))
    ...
    def _section_label(self, parent: Frame, text: str) -> None:
        ...
        Label(row, text=text, bg=BG, fg=DIM,
              font=(FONT_UI, 8, "bold"), padx=10).pack(side=LEFT)
    ...
    def _show_welcome(self) -> None:
        ...
        Label(hero, text="Bienvenido al instalador de",
              bg=BG, fg=MUTED, font=(FONT_UI, 11)).pack(anchor="w")
        Label(hero, text="NEMO  Memory Server",
              bg=BG, fg=TEXT, font=(FONT_UI, 22, "bold")).pack(anchor="w")
        Label(hero, text="Todo queda configurado automáticamente en unos minutos.",
              bg=BG, fg=DIM, font=(FONT_UI, 10)).pack(anchor="w", pady=(6, 0))
        ...
        Label(bar, text="Requisitos: macOS 12+  ·  Python 3.10+  ·  ~800 MB libres",
        ...
        self._btn(btn_row, "⚡  Instalación Express", self._start_install,
    ...
    def _show_configure(self) -> None:
        ...
        Label(f, text="Configurar instalación",
              bg=BG, fg=TEXT, font=(FONT_UI, 18, "bold")).pack(anchor="w")
        Label(f, text="Revisa las opciones y pulsa Instalar cuando estés listo.",
              bg=BG, fg=MUTED, font=(FONT_UI, 10)).pack(anchor="w", pady=(5, 4))
        ...
        Entry(row, textvariable=self.install_dir, bg=SURFACE2, fg=TEXT,
              insertbackground=TEXT, relief=FLAT, font=(FONT_UI, 10),
        ...
        for i, (name, info) in enumerate(EDITORS.items()):
            ...
            cb = Checkbutton(
                ...
                activeforeground=TEXT, font=(FONT_UI, 10),
            )
        ...
        Checkbutton(
            opt_box,
            text="⚡  Iniciar NEMO automáticamente con macOS (LaunchAgent)",
    ...
    def _show_installing(self) -> None:
        ...
        Label(left_hdr, text="PROGRESO", bg=SURFACE2, fg=MUTED,
              font=(FONT_UI, 8, "bold"), padx=20).pack(side=LEFT, pady=14)
        Label(left_hdr, text=f"{len(INSTALL_STEPS)} pasos", bg=SURFACE2, fg=MUTED,
              font=(FONT_UI, 8), padx=10).pack(side=RIGHT, pady=14)
        ...
        Label(log_hdr, text="▶  Registro en tiempo real", bg=SURFACE2, fg=MUTED,
              font=(FONT_UI, 9), padx=20).pack(side=LEFT, pady=14)

        self._log_widget = Text(
            right, bg=BG, fg=DIM,
            insertbackground=TEXT, relief=FLAT,
            font=(FONT_MONO, 8), wrap=WORD, padx=18, pady=14,
        ...
        self._progress_lbl = Label(prog, text="Iniciando…",
                                   bg=SURFACE2, fg=MUTED, font=(FONT_UI, 9), padx=16)
    ...
    def _show_done(self, success: bool) -> None:
        ...
        title_text = ("¡NEMO instalado con éxito!"
                      if success else "Instalación completada con advertencias")
        sub_text   = (
        ...
        Label(inner, text=title_text, bg=SURFACE, fg=TEXT,
              font=(FONT_UI, 15, "bold")).pack(anchor="w", pady=(2, 3))
        ...
        for idx, (key, label) in enumerate(INSTALL_STEPS):
            ...
            Label(row, text=_ICONS.get(r, "○"), bg=SURFACE2, fg=clr,
                  font=(FONT_UI, 11), width=2).pack(side=LEFT)
        ...
        if launchable:
            Label(f, text="ACCIONES RÁPIDAS", bg=BG, fg=MUTED,
                  font=(FONT_UI, 8, "bold")).pack(anchor="w", pady=(12, 6))
    ...
    def _open_log_window(self) -> None:
        ...
        txt = Text(top, bg=BG, fg=DIM, relief=FLAT,
                   font=(FONT_MONO, 9), padx=16, pady=12)
    ...
    def _run_install(self) -> None:  # noqa: C901
        ...
        src     = idir / "src"
        venv    = idir / ".venv"
        venv_py = venv / "bin" / "python"   # macOS / Linux venv path
        ...
        try:
            ...
            if not self.python_path:
                self._log("  → No encontrado. Intentando instalar via Homebrew…")
                if brew_install_python():
                    self.python_path = find_python()
            if self.python_path:
                self._log(f"  ✓ Encontrado: {self.python_path}")
                self._set_step("python", "done")
            else:
                self._log("  ✗ Python 3.10+ no disponible.")
                self._log("    Instala desde: https://python.org/downloads")
                self._log("    O con Homebrew: brew install python@3.12")
                self._set_step("python", "error")
                self.after(0, lambda: messagebox.showerror(
                    "Python no encontrado",
                    "Python 3.10 o superior no está instalado.\n\n"
                    "Instala desde:  https://python.org/downloads\n"
                    "o ejecuta:  brew install python@3.12\n\n"
                    "Luego vuelve a abrir el instalador.",
                ))
            ...
            with ThreadPoolExecutor(max_workers=4) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    with download_lock:
                        ...
                        self._set_progress(sub_pct,
                                           f"Descargando… {completed_count}/{len(CORE_FILES)}")
                        if ok:
            ...
            try:
                ...
                errors += 1
                venv_py = Path(self.python_path)  # fallback
            ...
            if is_ollama_installed():
                ...
                self._log("  → Intentando instalar Ollama via Homebrew…")
                brew = _find_brew()
                ollama_ok = False
                if brew:
                    try:
                        ...
                        if r_brew.returncode == 0 and is_ollama_installed():
                            self._log("  ✓ Ollama instalado via Homebrew")
                            self._set_step("ollama", "done")
                            ollama_ok = True
                    except Exception:
                        pass
                if not ollama_ok:
                    self._log("  → Homebrew no disponible o falló.")
                    self._log("    Abre https://ollama.ai/download/mac y descarga el instalador.")
                    try:
                        import webbrowser
                    ...
                    self.after(0, lambda: messagebox.showwarning(
                        ...
                        "Se ha abierto la descarga en tu navegador.\n"
                        "Instálalo y luego vuelve a abrir NEMO Installer.\n\n"
                        "Alternativa:  brew install ollama",
                    ))
                    self._set_step("ollama", "error")
                    errors += 1

            # ── 6. Embedding model (with streamed progress) ────────────────
            ...
            if not self._configured_editors:
                ...
                self._log("    Consulta: https://github.com/savantskie/persistent-ai-memory")
            ...
            try:
                self._create_mac_shortcuts(idir, venv_py, server)
    ...
    # ── macOS shortcuts & LaunchAgent ──────────────────────────────────────

    def _create_mac_shortcuts(
        self, idir: Path, venv_py: Path, server: Path,
    ) -> None:
        # Uninstall shell script
        uninstall_sh = idir / "uninstall.sh"
        uninstall_sh.write_text(
            "#!/bin/bash\n"
            'echo "Desinstalando NEMO Memory Server..."\n'
            f'rm -rf "{idir}"\n'
            "launchctl unload ~/Library/LaunchAgents/ai.nemo.server.plist 2>/dev/null\n"
            "rm -f ~/Library/LaunchAgents/ai.nemo.server.plist\n"
            'echo "NEMO desinstalado."\n',
            encoding="utf-8",
        )
        uninstall_sh.chmod(0o755)

        # Desktop .command shortcut (double-click → Terminal runs it)
        desktop = Path.home() / "Desktop"
        if desktop.exists():
            cmd_file = desktop / "Desinstalar NEMO.command"
            cmd_file.write_text(
                "#!/bin/bash\n"
                f'bash "{uninstall_sh}"\n',
                encoding="utf-8",
            ...
            self._log(f"  ✓ Acceso directo en escritorio: {cmd_file.name}")

        # LaunchAgent plist for autostart
        if self.autostart_var.get():
            la_dir = Path.home() / "Library" / "LaunchAgents"
            la_dir.mkdir(parents=True, exist_ok=True)
            plist = la_dir / "ai.nemo.server.plist"
            plist.write_text(
                ...
                '  <key>Label</key>\n'
                '  <string>ai.nemo.server</string>\n'
                '  <key>ProgramArguments</key>\n'
                '  <array>\n'
                f'    <string>{venv_py}</string>\n'
                f'    <string>{server}</string>\n'
                '  </array>\n'
                '  <key>RunAtLoad</key><true/>\n'
                '  <key>KeepAlive</key><false/>\n'
                '  <key>StandardErrorPath</key>\n'
                f'  <string>{idir}/nemo.log</string>\n'
                '</dict></plist>\n',
                encoding="utf-8",
            )
            subprocess.run(["launchctl", "load", str(plist)], capture_output=True)
            self._log(f"  ✓ LaunchAgent autostart: {plist}")
