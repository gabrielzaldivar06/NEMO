...
// ── (icons removed — pure orb design) ──────────────────────────────────────
const _DUMMY_ICON_START = { fact:       '',
...
const Graph = ForceGraph3D({ rendererConfig: { antialias: true, alpha: true } })(document.getElementById('graph-root'))
  ...
  .nodeThreeObject(n => {
    ...
    // Unique phase — no two nodes pulse in sync
    n.__phase  = (r * 0.031 + g * 0.019 + b * 0.013 + imp * 0.17) % (Math.PI * 2);
    // First letter of memory type for center label
    const LETTERS = { fact:'F', insight:'I', correction:'C', preference:'P', episodic:'E', procedure:'R', skill:'S', ai_memory:'M' };
    const letter  = LETTERS[n.mtype] || (n.mtype ? n.mtype[0].toUpperCase() : '?');

    n.__drawNode = function(t) {
      ctx.clearRect(0, 0, SZ, SZ);
      const cx = SZ / 2, cy = SZ / 2;
      const baseR = 32;
      // Very slow gentle breath — 7s cycle
      const br = 0.5 + 0.5 * Math.sin(t * 0.000714 + n.__phase);

      // ── Outer soft nebula glow ─────────────────────────────────────
      const glowR = baseR * (1.85 + br * 0.22);
      const glow  = ctx.createRadialGradient(cx, cy, baseR * 0.55, cx, cy, glowR);
      glow.addColorStop(0,    'rgba(' + r + ',' + g + ',' + b + ',' + (0.22 + br * 0.13).toFixed(3) + ')');
      glow.addColorStop(0.55, 'rgba(' + r + ',' + g + ',' + b + ',' + (0.06 + br * 0.04).toFixed(3) + ')');
      glow.addColorStop(1,    'rgba(' + r + ',' + g + ',' + b + ',0)');
      ctx.beginPath(); ctx.arc(cx, cy, glowR, 0, Math.PI * 2);
      ctx.fillStyle = glow; ctx.fill();

      // ── Glass sphere core ──────────────────────────────────────────
      const lx = cx - baseR * 0.30, ly = cy - baseR * 0.30;
      const rb = Math.min(255, r + 90), gb = Math.min(255, g + 90), bb = Math.min(255, b + 90);
      const rd = Math.floor(r * 0.4), gd = Math.floor(g * 0.4), bd = Math.floor(b * 0.4);
      const core = ctx.createRadialGradient(lx, ly, 0, cx, cy, baseR);
      core.addColorStop(0,    'rgba(255,255,255,' + (0.90 + br * 0.08).toFixed(3) + ')');
      core.addColorStop(0.18, 'rgba(' + rb + ',' + gb + ',' + bb + ',0.97)');
      core.addColorStop(0.52, 'rgba(' + r  + ',' + g  + ',' + b  + ',0.93)');
      core.addColorStop(0.85, 'rgba(' + rd + ',' + gd + ',' + bd + ',0.88)');
      core.addColorStop(1,    'rgba(' + Math.floor(r*0.15) + ',' + Math.floor(g*0.15) + ',' + Math.floor(b*0.15) + ',0.80)');
      ctx.beginPath(); ctx.arc(cx, cy, baseR, 0, Math.PI * 2);
      ctx.fillStyle = core; ctx.fill();

      // ── Specular highlight — bright flare top-left ─────────────────
      ctx.save();
      ctx.beginPath(); ctx.arc(cx, cy, baseR, 0, Math.PI * 2); ctx.clip();
      const sx = cx - baseR * 0.26, sy = cy - baseR * 0.26;
      const spec = ctx.createRadialGradient(sx, sy, 0, sx, sy, baseR * 0.38);
      spec.addColorStop(0,   'rgba(255,255,255,' + (0.78 + br * 0.12).toFixed(3) + ')');
      spec.addColorStop(0.4, 'rgba(255,255,255,' + (0.28 + br * 0.06).toFixed(3) + ')');
      spec.addColorStop(1,   'rgba(255,255,255,0)');
      ctx.fillStyle = spec; ctx.fillRect(0, 0, SZ, SZ);
      ctx.restore();

      // ── Type letter — subtle center label ─────────────────────────
      ctx.font = '700 ' + Math.round(baseR * 0.68) + 'px "DM Mono", monospace';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillStyle = 'rgba(255,255,255,' + (0.42 + br * 0.10).toFixed(3) + ')';
    ...
    const s   = 3.5 + imp * 1.5;
    sp.scale.set(s, s, 1);
    n.__sprite = sp; n.__canvas = canvas; n.__ctx = ctx; n.__tex = tex;
    return sp;
  })
  ...
  function tick(t) {
    if (t - lastT >= 33) {   // ~30fps
      ...
      if (total > 0) {
        ...
        // Textures flagged needsUpdate — graph's own rAF loop will render them
      }
    }
